...
public class CountDownProgressView extends TextView {
    ...
    public CountDownProgressView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
        TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.BYNAdSDKCountDownProgress);
        //TODO 圆实心的颜色属性
        if (typedArray.hasValue(R.styleable.BYNAdSDKCountDownProgress_circSolidColor)){
            //有这个属性索引 获取用户设置的颜色
            circSolidColor = typedArray.getColor(R.styleable.BYNAdSDKCountDownProgress_circSolidColor,0);
        }else{
            //没有这个属性索引 默认是浅灰色
            circSolidColor = typedArray.getColor(R.styleable.BYNAdSDKCountDownProgress_circSolidColor, Color.parseColor("#D3D3D3"));
        }

        //TODO 园边框颜色属性
        if (typedArray.hasValue(R.styleable.BYNAdSDKCountDownProgress_circFrameColor)){
            //有这个属性索引 获取用户设置的颜色
            circFrameColor = typedArray.getColor(R.styleable.BYNAdSDKCountDownProgress_circFrameColor,0);
        }else{
            //没有这个属性索引 默认是深灰色
            circFrameColor = typedArray.getColor(R.styleable.BYNAdSDKCountDownProgress_circFrameColor, Color.parseColor("#A9A9A9"));
        }

        //TODO 文字颜色属性
        if (typedArray.hasValue(R.styleable.BYNAdSDKCountDownProgress_textColor)){
            //有这个属性索引 获取用户设置的颜色
            textColor = typedArray.getColor(R.styleable.BYNAdSDKCountDownProgress_textColor,0);
        }else{
            //没有这个属性索引 默认是白色
            textColor = typedArray.getColor(R.styleable.BYNAdSDKCountDownProgress_textColor, Color.parseColor("#ffffff"));
        }

        //TODO 进度条颜色属性
        if (typedArray.hasValue(R.styleable.BYNAdSDKCountDownProgress_progressColor)){
            //有这个属性索引 获取用户设置的颜色
            progressColor = typedArray.getColor(R.styleable.BYNAdSDKCountDownProgress_progressColor,0);
        }else{
            //没有这个属性索引 默认是蓝色
            progressColor = typedArray.getColor(R.styleable.BYNAdSDKCountDownProgress_progressColor, Color.parseColor("#0000FF"));
        }
