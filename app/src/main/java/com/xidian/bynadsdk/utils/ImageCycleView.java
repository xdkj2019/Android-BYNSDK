...
public class ImageCycleView extends LinearLayout {
	...
	public ImageCycleView(Context context, AttributeSet attrs) {
		...
		LayoutInflater.from(context).inflate(R.layout.bynad_sdk_view_banner_content, this);
		...
		cGroup = (LinearLayout) findViewById(R.id.viewGroup1);
	}
	 ...
	 */
	public void setImageResources(ArrayList<ADInfo> infoList, ImageCycleViewListener imageCycleViewListener,int gravity) {
		// 清除所有子视图
		if (gravity == Gravity.CENTER){
			mGroup.setVisibility(GONE);
			cGroup.setVisibility(VISIBLE);
			mGroup = cGroup;
		}
		...
		for (int i = 0; i < imageCount; i++) {
			...
			if (i == 0) {
				mImageViews[i].setBackgroundResource(R.mipmap.bynad_sdk_weixuanzhong);
			} else {
				mImageViews[i].setBackgroundResource(R.mipmap.bynad_sdk_zhuanzhong);
			}
	...
	private final class GuidePageChangeListener implements OnPageChangeListener {
		...
		public void onPageSelected(int index) {
			...
			mImageIndex = index;
			index -= 1;
			mImageViews[index].setBackgroundResource(R.mipmap.bynad_sdk_zhuanzhong);
